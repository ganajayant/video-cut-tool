...
export const uploadVideos = async (req, res) => {
	...
	try {
		...
		db.run(query, [user.mediawikiId], (err, row) => {
			if (err) {
				console.log(err);
			}
			console.log(row);
		...
		const query2 = 'update User set refreshToken = ? where mediaWikiId = ?'

		db.run(query2, [newRefreshToken, user.mediawikiId], (err, row) => {
			if (err) {
				console.log(err);
			}
			console.log(row);
